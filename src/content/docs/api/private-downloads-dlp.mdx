...
An anonymous worker runs without yt-dlp's `--cookies` option. `codec` and `container` default to `auto`. `preferBetterAudio` defaults to false and `dubLanguage` defaults to `original`, so existing protocol-v2 clients remain valid.
