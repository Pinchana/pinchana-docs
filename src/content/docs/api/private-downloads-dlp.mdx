...
When the feature flag is off, configuration is incomplete, or the internal DLP API/Redis health check fails, `available` is `false`, `protocol` is `null`, and all option arrays are empty. Clients must fail closed and disable DLP submission. Older protocol-v2 instances may omit codec/container capability arrays; clients must then omit those optional submission fields.
...
3. **Submit once.** `POST /web/dlp/jobs/{jobId}/submit` with a URL, fixed quality, optional codec/container preferences, and optional `cookiesEnc`.
...
An anonymous worker runs without yt-dlp's `--cookies` option. `codec` and `container` default to `auto`, so existing protocol-v2 clients remain valid.
...
Unknown fields, arbitrary yt-dlp format strings, unsupported enum values, mismatched key IDs, replayed submissions, expired keys, oversized requests, and non-public URLs are rejected. Codec is a preference: if unavailable at the chosen quality ceiling, the worker falls back to the best available codec. `auto` chooses MP4 for H.264, WebM for AV1/VP9, or the source container when codec is also automatic. Explicit MP4/WebM/MKV requests remux without transcoding, so they preserve quality but cannot make an incompatible codec universally playable.
