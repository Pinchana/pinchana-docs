...
In the integrated `pinchana-api` deployment, put the global profile option before the Compose command: `docker compose --env-file .env --profile dlp up ...`. A normal `docker compose up -d` omits DLP by design. The standalone `pinchana-dlp` repository is a component-development stack and uses `REDIS_PASSWORD`; production uses the API repository's profile and `DLP_REDIS_PASSWORD`.
