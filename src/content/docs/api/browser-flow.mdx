...
6. **Normal scraper flow:** `POST /v1/web/scrape` sends the same bearer header and scrape request. It returns the normalized API v1 envelope with every `/media/...` value recursively rewritten to `/web/media/...`; `GET /web/media/{platform}/{post_id}/{filename}` streams the cached file. The legacy `/web/scrape` route remains available for older clients.
