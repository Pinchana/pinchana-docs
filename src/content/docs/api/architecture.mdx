...
POST /v1/scrape
  ...
  → normalize to the API v1 result and metadata envelopes
  → serve downloaded paths from the shared cache
```

`IN_PROCESS_PLUGINS` imports Python modules that register routers and is intended primarily for local development. The container registry always reads `MODULES_CONFIG` (default `/app/config/modules.yaml`) and assumes that configured services are already running. `CONTAINER_MODE=true` adds a separate Docker-socket-backed manager that can build, start, and stop them. The unversioned `/scrape` route remains available for legacy clients.
