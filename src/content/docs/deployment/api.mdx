...
5. Send an authenticated `/scrape`, then fetch its media URL with the same `X-API-Key` and a test `Range` header.
6. From the Web hostname, complete Turnstile and confirm `/web/verify`, scrape, and media playback through the Web BFF.
</Steps>
