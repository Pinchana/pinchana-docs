...
2. Pull and start: `docker compose --env-file .env pull && docker compose --env-file .env up -d`.
3. Inspect `docker compose --env-file .env ps` and `docker compose --env-file .env logs --tail=200 gluetun server`.
