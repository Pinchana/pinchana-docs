...
Common local failures are a missing API URL, mismatched site/secret Turnstile pair, hostname/action mismatch, a session secret shorter than 32 characters, unavailable API CORS identity response, missing synchronized FFmpeg assets, or DLP controls hidden because the API capability is disabled/unhealthy.
