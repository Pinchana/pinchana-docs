...
| YouTube Shorts | `youtube.com/shorts`, including `www` and mobile hosts | Machine `/scrape` uses the Shorts module; official Web routes the URL through YouTube DLP |
| SoundCloud | `soundcloud.com`, `on.soundcloud.com` | MP3 audio, title/artist, duration, and cover when available |
| YouTube Music | `music.youtube.com` | Machine `/scrape` uses the music module; official Web locks Audio only and uses DLP output options |
